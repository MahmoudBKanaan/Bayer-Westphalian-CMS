 ...
 * payment_history}, {@code paymenthistory}), {@code reminder_count} (alias {@code remindercount}),
 * {@code days_overdue} (alias {@code daysoverdue}), and {@code default_risk} (alias {@code
 * defaultrisk}).
 *
...
final class SegmentPaymentHistorySupport {
    ...
    private static boolean matchesNumericAggregate(
        ...
        return SegmentCriteria.matchesValue(
                operator, normalizedValue, Long.toString(aggregateValue));
    }
    ...
    private static PaymentStatus parsePaymentStatus(String rawValue) {
        ...
        throw new IllegalArgumentException("must be one of DUE, PAID, OVERDUE, or DEFAULT_RISK");
    }
