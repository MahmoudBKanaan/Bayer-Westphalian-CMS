 ...
 *
 * <p>{@code matchingCustomers} lists only eligibility-filtered (eligible) customers — never the raw
 * criteria set when exclusions apply (preview always applies EligibilityService). Size equals
 * {@code eligibleCount}.
 *
...
public record SegmentPreviewView(
    ...
    public SegmentPreviewView {
        ...
        if (eligibleCount > totalAudienceCount) {
            throw new IllegalArgumentException("eligibleCount must not exceed totalAudienceCount");
        }
        if (excludedCount > totalAudienceCount) {
            throw new IllegalArgumentException("excludedCount must not exceed totalAudienceCount");
        }
        if (eligibleCount + excludedCount != totalAudienceCount) {
            throw new IllegalArgumentException(
                    "eligibleCount + excludedCount must equal totalAudienceCount");
        }
        matchingCustomers = matchingCustomers == null ? List.of() : List.copyOf(matchingCustomers);
        exclusionReasonSummary =
                exclusionReasonSummary == null ? List.of() : List.copyOf(exclusionReasonSummary);
