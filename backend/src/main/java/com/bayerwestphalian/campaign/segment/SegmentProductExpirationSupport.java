...
final class SegmentProductExpirationSupport {
        ...
        return switch (operator) {
            ...
            case IN ->
                    splitInValues(normalizedValue).stream()
                            ...
                            .anyMatch(
                                    months ->
                                            hasAnyExpiringWithinMonths(ownerships, months, today));
            case BEFORE ->
                    ownerships.stream()
                            .anyMatch(
                                    ownership ->
                                            earliestMonthsUntilExpiration(ownership, today) >= 0
                                                    && earliestMonthsUntilExpiration(
                                                                    ownership, today)
    ...
    static boolean isExpiringWithinMonths(ProductOwnership ownership, int months, LocalDate today) {
    ...
    private static int parseNonNegativeInt(String rawValue) {
        try {
            ...
            if (value < 0) {
                throw new IllegalArgumentException(
                        "must be a non-negative integer number of months");
            }
            return value;
