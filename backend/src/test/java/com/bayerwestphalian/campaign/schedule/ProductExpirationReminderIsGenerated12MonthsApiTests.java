...
/**
 * API coverage for KB item 400 / BR-023: product-expiration reminder is generated 12 months before
 * expiration via {@code POST /api/reminders/expiration/12-month/generate}.
 */
...
class ProductExpirationReminderIsGenerated12MonthsApiTests {

    private static final UUID REMINDER_ID = UUID.fromString("90000000-0000-0000-0000-000000000400");
    ...
    @Test
    void campaignManagerCanGenerateTwelveMonthProductExpirationReminderViaPostApi()
            throws Exception {
        when(jwtService.validateToken("campaign-token", JwtTokenType.ACCESS))
                .thenReturn(roleClaims(SystemRoleName.CAMPAIGN_MANAGER));
        when(reminderService.generateTwelveMonthExpirationReminders(eq(AS_OF_DATE)))
                .thenReturn(List.of(twelveMonthReminderView()));
        ...
        verify(reminderService).generateTwelveMonthExpirationReminders(AS_OF_DATE);
    }

    @Test
    void adminCanGenerateTwelveMonthProductExpirationReminder() throws Exception {
    ...
    @Test
    void customerServiceAgentCanGenerateTwelveMonthProductExpirationReminder() throws Exception {
    ...
    @Test
    void twelveMonthGenerateAllowsNullAsOfDate() throws Exception {
        when(jwtService.validateToken("campaign-token", JwtTokenType.ACCESS))
                .thenReturn(roleClaims(SystemRoleName.CAMPAIGN_MANAGER));
        when(reminderService.generateTwelveMonthExpirationReminders(isNull()))
                .thenReturn(List.of(twelveMonthReminderView()));
        ...
        verify(reminderService).generateTwelveMonthExpirationReminders(isNull());
    }

    @Test
    void twelveMonthGenerateReturnsEmptyListWhenNoCandidates() throws Exception {
        when(jwtService.validateToken("campaign-token", JwtTokenType.ACCESS))
                .thenReturn(roleClaims(SystemRoleName.CAMPAIGN_MANAGER));
        when(reminderService.generateTwelveMonthExpirationReminders(eq(AS_OF_DATE)))
                .thenReturn(List.of());
    ...
    void unauthorizedRolesCannotGenerateTwelveMonthReminders(SystemRoleName role) throws Exception {
                ...
                .andExpect(
                        content()
                                .string(
                                        not(
                                                containsString(
                                                        "Twelve-month product-expiration reminders generated"))));

        verify(reminderService, never()).generateTwelveMonthExpirationReminders(any());
    }
    ...
    private static JwtTokenClaims roleClaims(SystemRoleName role) {
        return new JwtTokenClaims(
                UUID.fromString("10000000-0000-0000-0000-000000000400"),
                "test.user@bayer-westphalian.test",
                List.of(role));
    }

    private static ReminderScheduleView twelveMonthReminderView() {
