...
/**
 * API coverage for KB item 398 / BR-023: product-expiration reminder is generated 3 months before
 * expiration via {@code POST /api/reminders/expiration/3-month/generate}.
 */
...
class ProductExpirationReminderIsGenerated3MonthsApiTests {

    private static final UUID REMINDER_ID = UUID.fromString("90000000-0000-0000-0000-000000000398");
    ...
    @Test
    void campaignManagerCanGenerateThreeMonthProductExpirationReminderViaPostApi()
            throws Exception {
        when(jwtService.validateToken("campaign-token", JwtTokenType.ACCESS))
                .thenReturn(roleClaims(SystemRoleName.CAMPAIGN_MANAGER));
        when(reminderService.generateThreeMonthExpirationReminders(eq(AS_OF_DATE)))
                .thenReturn(List.of(threeMonthReminderView()));
        ...
        verify(reminderService).generateThreeMonthExpirationReminders(AS_OF_DATE);
    }

    @Test
    void adminCanGenerateThreeMonthProductExpirationReminder() throws Exception {
    ...
    @Test
    void customerServiceAgentCanGenerateThreeMonthProductExpirationReminder() throws Exception {
    ...
    @Test
    void threeMonthGenerateAllowsNullAsOfDate() throws Exception {
        when(jwtService.validateToken("campaign-token", JwtTokenType.ACCESS))
                .thenReturn(roleClaims(SystemRoleName.CAMPAIGN_MANAGER));
        when(reminderService.generateThreeMonthExpirationReminders(isNull()))
                .thenReturn(List.of(threeMonthReminderView()));
        ...
        verify(reminderService).generateThreeMonthExpirationReminders(isNull());
    }

    @Test
    void threeMonthGenerateReturnsEmptyListWhenNoCandidates() throws Exception {
        when(jwtService.validateToken("campaign-token", JwtTokenType.ACCESS))
                .thenReturn(roleClaims(SystemRoleName.CAMPAIGN_MANAGER));
        when(reminderService.generateThreeMonthExpirationReminders(eq(AS_OF_DATE)))
                .thenReturn(List.of());
    ...
    void unauthorizedRolesCannotGenerateThreeMonthReminders(SystemRoleName role) throws Exception {
                ...
                .andExpect(
                        content()
                                .string(
                                        not(
                                                containsString(
                                                        "Three-month product-expiration reminders generated"))));

        verify(reminderService, never()).generateThreeMonthExpirationReminders(any());
    }
    ...
    private static JwtTokenClaims roleClaims(SystemRoleName role) {
        return new JwtTokenClaims(
                UUID.fromString("10000000-0000-0000-0000-000000000398"),
                "test.user@bayer-westphalian.test",
                List.of(role));
    }

    private static ReminderScheduleView threeMonthReminderView() {
