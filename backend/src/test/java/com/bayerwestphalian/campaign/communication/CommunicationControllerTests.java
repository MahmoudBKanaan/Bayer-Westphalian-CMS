...
class CommunicationControllerTests {
    ...
    private static final UUID EVENT_ID = UUID.fromString("63000000-0000-0000-0000-000000000333");
    private static final UUID CUSTOMER_ID = UUID.fromString("20000000-0000-0000-0000-000000000333");
    private static final UUID CAMPAIGN_ID = UUID.fromString("50000000-0000-0000-0000-000000000333");
    ...
    void exposesContactEventApiRoute() {
        assertThat(CommunicationController.class.isAnnotationPresent(RestController.class))
                .isTrue();
    ...
    @Test
    void exposesRecordContactEventPostEndpointWithManageAuthorization() throws Exception {
        Method method =
                CommunicationController.class.getMethod(
                        "recordContactEvent", RecordContactEventRequest.class);
        PostMapping postMapping = method.getAnnotation(PostMapping.class);
        PreAuthorize preAuthorize = method.getAnnotation(PreAuthorize.class);

        assertThat(postMapping).isNotNull();
        assertThat(preAuthorize.value())
                .isEqualTo("@authz.hasAnyRole('ADMIN', 'CUSTOMER_SERVICE_AGENT', 'SALES_AGENT')");
    }

    @Test
    void recordsContactEvent() throws Exception {
        when(communicationService.recordContactEvent(any(RecordContactEventCommand.class)))
                .thenReturn(contactEventView());
        ...
        mockMvc.perform(
                        post("/api/contact-events")
                                ...
                                .content(
                                        ...
                                        """
                                                .formatted(CUSTOMER_ID.toString())))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.message").value("Contact event recorded"))
                .andExpect(jsonPath("$.data.id").value(EVENT_ID.toString()));

        ArgumentCaptor<RecordContactEventCommand> commandCaptor =
                ArgumentCaptor.forClass(RecordContactEventCommand.class);
        verify(communicationService).recordContactEvent(commandCaptor.capture());
        assertThat(commandCaptor.getValue().customerId()).isEqualTo(CUSTOMER_ID);
        assertThat(commandCaptor.getValue().channel()).isEqualTo(CommunicationChannel.EMAIL);
        assertThat(commandCaptor.getValue().eventType()).isEqualTo(ContactEventType.SENT);
        assertThat(commandCaptor.getValue().occurredAt())
                .isEqualTo(Instant.parse("2026-07-10T15:00:00Z"));
    }
