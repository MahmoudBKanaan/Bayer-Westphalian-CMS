...
class SegmentCanFilterByPaymentHistoryTests {
    ...
    @Test
    void campaignManagerCanPreviewAudienceFilteredByPaymentStatus() throws Exception {
        ...
        mockMvc.perform(
                        post("/api/segments/preview")
                                ...
                                .content(paymentStatusPreviewPayload("overdue")))
        ...
        assertThat(commandCaptor.getValue().criteria().getFirst().value()).isEqualTo("overdue");
    }

    @Test
    void biAnalystCanPreviewAudienceWithoutDefaultRiskPayments() throws Exception {
    ...
    @Test
    void campaignManagerCanPreviewAudienceFilteredByReminderCountAndDefaultRisk() throws Exception {
        when(jwtService.validateToken("campaign-manager-token", JwtTokenType.ACCESS))
                .thenReturn(campaignManagerClaims());
        when(segmentService.previewSegment(any(SegmentPreviewCommand.class)))
                .thenReturn(SegmentPreviewView.of(1, List.of(customerView())));
    ...
    @Test
    void biAnalystCanPreviewAudienceFilteredByDaysOverdue() throws Exception {
        when(jwtService.validateToken("bi-analyst-token", JwtTokenType.ACCESS))
                .thenReturn(biAnalystClaims());
        when(segmentService.previewSegment(any(SegmentPreviewCommand.class)))
                .thenReturn(SegmentPreviewView.of(1, List.of(customerView())));
        ...
        assertThat(commandCaptor.getValue().criteria().getFirst().operator())
                .isEqualTo(SegmentOperator.AFTER);
        assertThat(commandCaptor.getValue().criteria().getFirst().value()).isEqualTo("7");
    }

    @Test
    void campaignManagerCanPreviewAudienceFilteredByPaymentStatusInList() throws Exception {
        when(jwtService.validateToken("campaign-manager-token", JwtTokenType.ACCESS))
                .thenReturn(campaignManagerClaims());
        when(segmentService.previewSegment(any(SegmentPreviewCommand.class)))
        ...
        assertThat(commandCaptor.getValue().criteria().getFirst().value())
                .isEqualTo("OVERDUE,DEFAULT_RISK");
    }

    @Test
    void campaignManagerCanCreateSegmentWithPaymentHistoryCriteria() throws Exception {
        when(jwtService.validateToken("campaign-manager-token", JwtTokenType.ACCESS))
                .thenReturn(campaignManagerClaims());
        when(segmentService.createSegment(any(CreateSegmentCommand.class)))
                .thenReturn(
                        new SegmentView(
                                UUID.fromString("42000000-0000-0000-0000-000000000031"),
                                "Overdue payers",
        ...
        mockMvc.perform(
                        post("/api/segments")
                                ...
                                .content(
                                        ...
                                        {
                                          "name": "Overdue payers",
                                          "description": "Payment status OVERDUE",
                                          "visibility": "TEAM",
                                          "criteria": [
                                            {
                                              "fieldName": "payment_status",
        ...
        assertThat(commandCaptor.getValue().criteria().get(1).fieldName())
                .isEqualTo("days_overdue");
        assertThat(commandCaptor.getValue().criteria().get(1).value()).isEqualTo("0");
    }
    ...
    private static String paymentStatusPreviewPayload(String paymentStatusValue) {
        return """
                {
                  "criteria": [
                    {
