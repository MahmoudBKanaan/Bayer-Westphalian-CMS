...
class SegmentCanFilterByBehaviorStatusTests {
    ...
    @Test
    void campaignManagerCanPreviewAudienceFilteredByCustomerStatus() throws Exception {
        ...
        when(segmentService.previewSegment(any(SegmentPreviewCommand.class)))
                .thenReturn(SegmentPreviewView.of(1, List.of(interestedCustomer())));

        mockMvc.perform(
                        post("/api/segments/preview")
                                .header("Authorization", "Bearer campaign-manager-token")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(statusPreviewPayload("interested")))
        ...
        assertThat(commandCaptor.getValue().criteria().getFirst().fieldName()).isEqualTo("status");
        assertThat(commandCaptor.getValue().criteria().getFirst().value()).isEqualTo("interested");
    }

    @Test
    void biAnalystCanPreviewAudienceFilteredByBehaviorAlias() throws Exception {
    ...
    @Test
    void campaignManagerCanPreviewAudienceFilteredByDoNotContactAndInterestSource()
            throws Exception {
        when(jwtService.validateToken("campaign-manager-token", JwtTokenType.ACCESS))
                .thenReturn(campaignManagerClaims());
        when(segmentService.previewSegment(any(SegmentPreviewCommand.class)))
    ...
    private static String statusPreviewPayload(String statusValue) {
        return """
                {
                  "criteria": [
                    {
    ...
    private static CustomerView interestedCustomer() {
