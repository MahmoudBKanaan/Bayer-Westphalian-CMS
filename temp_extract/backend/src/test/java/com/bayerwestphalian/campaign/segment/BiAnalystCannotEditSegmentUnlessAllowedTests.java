 ...
 *
 * <p>Proves {@code BI_ANALYST} alone cannot update, delete, or manage segment definitions
 * (HTTP filter + {@code canManageSegments} / {@code canCreateSegments}), while still reading and
...
class BiAnalystCannotEditSegmentUnlessAllowedTests {
    ...
    private static final UUID BI_USER_ID =
    ...
    void biAnalystCannotUpdateSegmentViaPutApi() throws Exception {
        ...
        verify(segmentService, never()).updateSegment(any(UUID.class), any(UpdateSegmentCommand.class));
    }
    ...
    private static void assertMethodUsesCanManageSegments(String methodName, Class<?>... parameterTypes)
            throws Exception {
