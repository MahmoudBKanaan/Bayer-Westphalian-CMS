 ...
 * city=Munich) OR city=Berlin}. Pure multi-criterion OR chains use OR on every join after the first.
 */
final class SegmentCriteriaLogicSupport {
    ...
    /**
     * Pure OR over match results: empty list matches (no constraints); otherwise at least one result
     * must be true (KB FR-078 OR criteria logic).
     */
     ...
     *
     * <p>{@code joinOperators[i]} links {@code matchResults[i]} to the accumulated result for {@code
     * i > 0}. Sizes must match; empty input matches.
     */
    static boolean evaluate(
            List<Boolean> matchResults, List<SegmentJoinOperator> joinOperators) {
        if (matchResults == null || matchResults.isEmpty()) {
            return true;
        }
    ...
    /**
     * True when every criterion after the first uses {@link SegmentJoinOperator#OR} (pure OR chain).
     * The first criterion's join operator is ignored under left-to-right join semantics.
     */
