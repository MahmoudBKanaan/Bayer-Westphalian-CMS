...
public class Campaign extends BaseEntity {
    ...
    public void updateMessage(String messageSubject, String messageBody) {
        ...
        if (messageSubject != null && messageSubject.length() > 255) {
            throw new IllegalArgumentException("Campaign message subject must not exceed 255 characters");
        }
    ...
    public Campaign submit() {
        if (status != CampaignStatus.DRAFT && status != CampaignStatus.REJECTED) {
            throw new IllegalStateException(
                    "Only DRAFT or REJECTED campaigns can be submitted; current status is " + status);
        }
    ...
    public Campaign reject(String reason, String complianceReviewNotes) {
        ...
        this.rejectionReason = requireNonBlank(reason, "Rejection reason");
        this.complianceReviewNotes = normalizeOptional(complianceReviewNotes);
        this.approvedBy = null;
        this.approvedAt = null;
        return this;
    ...
    public Campaign complete() {
        if (status != CampaignStatus.ACTIVE && status != CampaignStatus.PAUSED) {
            throw new IllegalStateException(
                    "Only ACTIVE or PAUSED campaigns can be completed; current status is " + status);
        }
    ...
    /**
     * Draft and rejected campaigns may be edited by the owner/campaign manager workflow.
     * Submitted and later statuses require proper workflow transitions (item 247 later).
     */
