...
final class SegmentExclusionReasonSummarySupport {
    ...
    /**
     * Builds a summary list from ineligible decisions only. Results are ordered by descending count,
     * then ascending reason code for stable API output.
     */
    static List<SegmentExclusionReasonSummary> summarize(List<EligibilityDecision> decisions) {
        ...
        for (EligibilityDecision decision : decisions) {
            ...
            MutableSummary summary = byCode.computeIfAbsent(code, key -> new MutableSummary(message));
