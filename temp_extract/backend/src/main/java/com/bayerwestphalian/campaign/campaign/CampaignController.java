...
public class CampaignController {
    ...
    public ResponseEntity<ApiResponse<List<CampaignView>>> listCampaigns(
        ...
        List<CampaignView> campaigns =
                campaignService.searchCampaigns(searchRequest.toCriteria());
     ...
     *
     * <p>{@code GET /api/campaigns/{id}} returns the full campaign definition for roles with
     * {@code canReadCampaigns}: status, owner, segment, channel, message, schedule, approval fields,
    ...
    public ResponseEntity<ApiResponse<List<CampaignRecipientView>>> listEligibleRecipients(
        ...
        return ResponseEntity.ok(ApiResponse.success("Eligible campaign recipients loaded", recipients));
    }
    ...
    public ResponseEntity<ApiResponse<List<CampaignRecipientView>>> listExcludedRecipients(
        ...
        return ResponseEntity.ok(ApiResponse.success("Excluded campaign recipients loaded", recipients));
    }
    ...
    public ResponseEntity<ApiResponse<CampaignRecipientSummaryView>> summarizeCampaignRecipients(
        ...
        return ResponseEntity.ok(
                ApiResponse.success("Campaign recipient summary loaded", summary));
    }

    /**
     * Submit campaign endpoint (KB item 228 / FR-058).
     *
     * <p>{@code POST /api/campaigns/{id}/submit} moves a draft or rejected campaign into
     * {@code SUBMITTED} status for compliance review.
     */
     ...
     *
     * <p>{@code POST /api/campaigns/{id}/approve} moves a submitted campaign into
     * {@code APPROVED} status. Optional body may include compliance review notes (item 231).
     * Approval writes an {@code APPROVE} audit log for entity type {@code campaigns} (item 235).
     */
    @PostMapping("/{id}/approve")
    @PreAuthorize("@authz.canReviewCampaigns()")
    public ResponseEntity<ApiResponse<CampaignView>> approveCampaign(
            @PathVariable UUID id,
            @RequestBody(required = false) ApproveCampaignRequest request) {
