...
final class SegmentProductOwnershipSupport {
    ...
    private static boolean matchesProductType(
        ...
        return switch (operator) {
            case EQUALS -> ownedTypes.stream().anyMatch(type -> type.equalsIgnoreCase(normalizedValue));
            case NOT_EQUALS ->
            ...
            case IN ->
                    ownedProductIds.stream()
                            .anyMatch(
                                    id ->
                                            splitInValues(normalizedValue).stream()
                                                    .anyMatch(filter -> filter.equalsIgnoreCase(id)));
            default -> false;
        };
    }
    ...
    private static boolean matchesOwnershipStatus(
        ...
        boolean matchesStatus =
                switch (status) {
                    ...
                    case EXPIRED ->
                            ownerships.stream()
                                    .anyMatch(ownership -> ownership.getStatus() == OwnershipStatus.EXPIRED);
                    case CANCELLED ->
                            ownerships.stream()
                                    .anyMatch(ownership -> ownership.getStatus() == OwnershipStatus.CANCELLED);
                };
